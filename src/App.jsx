...
import { flashcards, quizQuestions, topics } from './data'

const topicsData = flashcards
...
function App() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('ancientIndiaProgress')
    if (saved) {
      const data = JSON.parse(saved)
      setMasteredTopics(data.masteredTopics || [])
    }

    const savedQuiz = localStorage.getItem('ancientIndiaQuizState')
  ...
  const saveProgress = (mastered) => {
    localStorage.setItem('ancientIndiaProgress', JSON.stringify({
      masteredTopics: mastered
    }))
  }

  const saveQuizState = (index, currentScore, answers) => {
    localStorage.setItem('ancientIndiaQuizState', JSON.stringify({
      currentIndex: index,
      score: currentScore,
      quizAnswers: answers
    }))
  ...
  const resetQuiz = () => {
    localStorage.removeItem('ancientIndiaQuizState')
    setCurrentIndex(0)
    setScore(0)
    setQuizAnswers([])
  ...
  const getFilteredTopics = () => {
    ...
    } else if (selectedTopic === 'buddhism') {
      filtered = topicsData.filter(v => v.category === 'buddhism')
    } else if (selectedTopic === 'jainism') {
    ...
    } else if (selectedTopic === 'sangam') {
      filtered = topicsData.filter(v => v.category === 'sangam')
    }
  ...
  const renderTopicMenu = () => {
    const neolithicCount = topicsData.filter(t => t.category === 'neolithic').length
    ...
    const buddhismCount = topicsData.filter(t => t.category === 'buddhism').length
    const buddhismQuiz = quizQuestions.filter(q => q.category === 'buddhism').length
    const jainismCount = topicsData.filter(t => t.category === 'jainism').length
    ...
    const sangamCount = topicsData.filter(t => t.category === 'sangam').length
    const sangamQuiz = quizQuestions.filter(q => q.category === 'sangam').length
    ...
    return (
      <div className="mode-selector">
        ...
        <div className="mode-card" onClick={() => setSelectedTopic('gupta')}>
          <div className="mode-icon">🏛️</div>
          <h3>Gupta Empire</h3>
        ...
        <div className="mode-card" onClick={() => setSelectedTopic('sangam')}>
          <div className="mode-icon">📜</div>
          <h3>Sangam Age</h3>
          <p>{sangamCount} flashcards · {sangamQuiz} quiz questions</p>
        </div>
      </div>
    )
  }
    ...
    <div className="app">
      <div className="container">
        <div className="header">
          <h1>🏛️ Ancient India - UPSC Prep</h1>
          <p>Master Neolithic Sites, Indus Valley & Ancient History</p>
        </div>
